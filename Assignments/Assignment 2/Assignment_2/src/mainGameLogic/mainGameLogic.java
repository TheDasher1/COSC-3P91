...
public class mainGameLogic {
    ...
    public void startGame() {
        ...
        for (Vehicle v : map.getAllVehiclesOnMap()) { // processes all the vehicles on the map
            ...
            if (v.getCurrPosition() > map.getRoad(v.getCurrRoad().getRoadID()).getMaxVehiclesPerLane()) { // player has reached the end of this road and has arrived at an intersection, should decide which road they should go into from this intersection
                ...
                v.updateCurrIntersection(map.getVehicle(v.getVehicleID()).getCurrIntersection()); // set it's location to be on an intersection
                
                int intsectChoise, lChoice;

                System.out.print("\nThere are " + v.getCurrIntersection().getRoadsAttached().size() + " roads attached to this intersection, which to turn into: ");
                intsectChoise = input.nextInt();
                ...
                v.updateCurrRoad(map.getRoad(intsectChoise)); // sets the vehicle to be on the road

                map.getRoad(v.getCurrRoad().getRoadID()).addVehicle(v, lChoice); // updates the road to hold the vehicle
    ...
    public void VehicleMaker(int pID) {
        ...
        try {
            ...
            if (vID == 0) {
                ...
                map.createCar(usedIDs, vStartSpeed, map.getRoad(vStartRoadID), vStartLane);

                this.usedVehicleIDList.put(usedIDs, true);

                players.get(pID).changeVehicle(map.getVehicle(usedIDs));
                
            
            } else {
                if (!this.usedVehicleIDList.containsKey(vID)) {
                    map.createCar(vID, vStartSpeed, map.getRoad(vStartRoadID), vStartLane);

                    this.usedVehicleIDList.put(vID, true);

                    players.get(pID).changeVehicle(map.getVehicle(vID));
